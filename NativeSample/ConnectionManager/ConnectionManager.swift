...
typealias responseHandler = (_ response: [String: Any]?, _ status: StatusCode, _ error: Error?) -> Void
...
public class ConnectionManager: NSObject {
    ...
    private func createRequest(_ method: MethodType = .POST, _ urlString: String, _ parameters: [String: Any]? = nil, _ isMultipartRequest: Bool = false, _ imagesData: [Any]? = nil, _ type: Type = .jpeg) -> URLRequest? {
        ...
        do {
            ...
            if let parameterRequest = parameters {
                var postData: Data? = nil
                if isMultipartRequest {
                    postData =  createBodyWithParameters(parameterRequest, imagesData, type, boundary)
                }
                else {
                    postData = try JSONSerialization.data(withJSONObject: parameterRequest, options: JSONSerialization.WritingOptions())
                }
    ...
    private func handleResponse(_ data: Data?, _ response: URLResponse?, _ error: Error?, _ completionHandler: @escaping responseHandler) {
        ...
        DispatchQueue.main.async {
            ...
            do {
                ...
                guard let responseDictionary = try JSONSerialization.jsonObject(with: responseData, options: .mutableLeaves) as? [String : Any] else {
                    completionHandler(nil, .failed, error)
                    return
                }
                print("Response:\(responseDictionary)")
                completionHandler(responseDictionary, .success , nil)
            } catch {
                completionHandler(nil, .failed, error)
            }
        }
